...
data_05_13<- read.csv(file.path(dir_anx,"_raw_data/NOEP/NewEngland_sectors_counties-2.csv"),
                stringsAsFactors=F)%>%
...
data_90_04 <- read.csv(file.path(dir_anx,"_raw_data/NOEP/NewEngland_sectors_counties-3.csv"),
                stringsAsFactors=F)%>%
        filter(Sector=='Tourism & Recreation')%>%
        mutate(Employment = as.numeric(gsub(",","",.$Employment)),
               Establishments = as.numeric(gsub(",","",.$Establishments)))

all = rbind(data_05_13,data_90_04)

DT::datatable(all, rownames=F, caption = 'Employment in the Tourism & Recreation sector of Coastal Jobs in New England provided by NOEP')
...
write.csv(jobs_5yr,'tr_scores.csv')
