...
bls.df <- data.frame(year=sapply(bls.json$Results$series[[1]]$data,"[[","year"),
            ...
            mutate(state  = codes$state[match(code,codes$code)],
                   county = codes$county[match(code,codes$code)])
}

# match the data with state and county

dt <- lapply(codes$code,get_data)
bls_wages <- do.call(rbind,dt)%>%
