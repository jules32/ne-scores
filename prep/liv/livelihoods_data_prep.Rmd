...
The Livelihoods sub-goal is calculated using Ocean Economy employment and wage data from the [National Ocean Economics Program](http://www.oceaneconomics.org/). There are two components to this goal model, **jobs (or employment)** and **wages**. NOEP provides data on the number of jobs, and total wages for jobs that directly or indirectly depend upon the ocean. Annual job and wage growth is measured by comparing total employment and mean annual wages to the average employment and wages of the previous 3 years. The target for region job growth is set to be equal to or greater than the national average, calculated using data from the Bureau of Labor Statistics. The target for mean annual wage growth is set at 3.5%, reflecting the [*Nominal Wage Growth Target*](http://www.epi.org/nominal-wage-tracker/) as set by the Federal Reserve.
...
$$j ' = \frac{\frac{C_{i}}{C_{r}}}{\frac{A_{i}}{A_{r}}}$$

where $C_{i}$ is the number of coastal jobs in year *i*, $C_{r}$ is the average number of coastal jobs over the previous 3 years (*i* - 4 to *i* - 1), ${A_{i}}$ is the total number of jobs nationwide for year *i* and ${A_{r}}$ is the average number of jobs nationwide over the previous 3 years.
...
Our reference point is the average number of jobs over the previous 3 years.
...
jobs_cst_ref <- coast_jobs %>%
  arrange(year) %>%
  group_by(rgn_id) %>%
  mutate(coast_jobs_3yr = rollapply(rgn_employment, 3, FUN=mean, align = "right", na.rm=F, partial = T),
         coast_jobs_prev_3yr = lag(coast_jobs_3yr, n = 1)) %>%
  filter(year > 2007) %>%
  select(state, year, rgn_id, rgn_name, ref = coast_jobs_prev_3yr)
...
$$g ' = \frac{\frac{W_{i}}{W_{r}}}{{1.035}}$$

where $W_{i}$ is the mean annual wage for regional coastal employment in year *i*, and $W_{r}$ is the mean wage for coastal employment over the previous 3 years (*i* - 4 to *i* - 1). The change in wages (${\frac{W_{i}}{W_{r}}}$) is then divided by the target, 1.035, representing the 3.5% Nominal Wage Growth Target established by the Federal Reserve.
...
wages_cst_ref <- coast_wages %>%
  arrange(year) %>%
  group_by(rgn_id) %>%
  mutate(coast_wages_3yr = rollapply(wages_per_job, 3, FUN=mean, align = "right", na.rm=F, partial = T),
         coast_wages_prev_3yr = lag(coast_wages_3yr, n = 1)) %>%
  filter(year > 2007) %>%
  select(state, year, rgn_id, rgn_name, ref = coast_wages_prev_3yr)
...
ggplot(c, aes(x = year, y = wages, color = data)) +
  ...
  scale_color_manual(" ", labels = c("Mean wages over the \nprevious 3 years","Annual average wages"), values = c("red", "blue"))
```
...
us_employment <- bls_api("ENUUS00010010",
  ...
  mutate(us_jobs_3yr = rollapply(us_jobs, 3, FUN=mean, align = "right", na.rm=F, partial = T),
         ...
         us_job_growth = us_jobs/us_jobs_prev_3yr) %>%
  select(year, us_job_growth)

ggplot(us_employment, aes(x = year, y = us_job_growth)) +
  geom_line() +
  ylab("Job growth compared to previous 3 years")
