## Fuctions to support the Biodiversity goal for OHI Northeast

message('NOTE: functions.R requires that the following variables be set in the global environment (main script):')
...
spp_rgn2cell <- function(poly_rgn,
  ...
  ### load Aquamaps LOICZID raster already cropped to the Northeast region (in create_half_deg_cell_raster.R)
  loiczid_raster       <- raster::raster("prep/bio/data/loiczid.tif")

  rgn2cell_file <- file.path(dir_goal, sprintf('data/cell_id_by_rgn%s.csv', rgn_tag))
...
spp_append_bcsee <- function(spp_all) {
  bcsee_file <- file.path(dir_M, 'git-annex/bcprep/_raw_data',
                          'bcsee/bcsee_export/d2016/bcsee_export.tsv')
