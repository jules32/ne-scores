...
foreach(f = sst_global_files) %dopar% {
  ...
  raster(f)%>%                     #raster the file
    ...
    mask(ocean_ne, filename = paste0('int/sst_annual_anoms/annual_anoms_',substr(basename(f),25,28),'.tif'),overwrite=T)
    
}

plot(raster('int/sst_annual_anoms/annual_anoms_2012.tif'),col=cols,axes=F,main = "Sea Surface Temperature 2012",
     legend.args=list(text='Anomalous Weeks', side=4, font=2, line=2.5, cex=0.8))
```
...
s        <- lapply(list.files('int/sst_annual_anoms',full.names=T),raster)%>%brick()
...
l <- list.files('int/sst_annual_anoms',full.names=T)
...
for(i in 1982:2008){ #i=2005
  ...
  writeRaster(s,filename = paste0('int/sst_5yr_anom_sums/sum_anoms_',min(yrs),'-',max(yrs),'.tif'),overwrite=T)
}
...
sst_aggs <- list.files('int/sst_5yr_anom_sums',full.names=T) 
...
resc_func <- function(x){
  ...
  calc(.,fun=function(x){ifelse(x<0,0,ifelse(x>130,1,x/130))},
         filename = paste0('int/sst_rescale/sst_rescale_',yrs,'.tif'),overwrite=T)
}

#mclapply is used for parallel processing across multiple cores
mclapply(sst_aggs,resc_func,mc.cores = 10)
...
resc      <- lapply(list.files('int/sst_rescale',full.names=T),raster)%>%brick()
...
pressure_stack <- lapply(list.files('int/sst_rescale',full.names=T),raster)%>%brick()
