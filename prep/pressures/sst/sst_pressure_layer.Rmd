...
foreach(f = sst_global_files) %dopar% {
  ...
  raster(f)%>%                     #raster the file
    ...
    projectRaster(ocean_ne, filename = paste0('int/sst_anom_diffs/sst_anom_diffs_',substr(f,77,85),'.tif'),overwrite=T)
...
Since we aren't sure what spatial reference point is most appropriate for the region yet, we are going to use two different ones:

1. The maximum number of anomalies above the reference period (1985-1989) from a cell that lies within our region bounding box
2. The maximum number of anomalies above the reference period (1985-1989) a cell has anywhere in the world.

For each of these reference points we will set the max from the entire time series. The spatial patterns and relative scores between cells will not change between the two reference points. The main difference will be the level at which we assign a pressure value of 1. If the global reference point is much higher than the local reference point, our region pressure scores will be much lower. But if we use a local reference point, we are more likely to get higher pressure scores which will ultimately have a great impact on goals where SST acts as a pressure.
...
local_resc <- function(x){
  ...
  #if a cell value is greater than or equal to the reference point, we assign a value of 1, otherwise it is divided by the reference point
  raster(x)%>%
...
ref_point     <- max(sst_global_files%>%
                   stack()%>%
                   cellStats(stat='max'))
```
...
#define the rescaling function to apply to the files
...
#mclapply is used for parallel processing
...
Using a global reference point, we can see how the pressure changes over time.

```{r global_pressure_gif,eval=F}
