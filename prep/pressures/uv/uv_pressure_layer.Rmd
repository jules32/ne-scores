...
We define the reference point as more than half the weeks in a 5 year period are anomalous, indicating a regime shift. This is equal to (52x5)/2, or **greater than 130**.
...
foreach(i = c(2005:2012)) %dopar% { #i=2005
 ...
 yrs <- c(i:(i+4))
  ...
  out_rescale = out_files[substr(out_files,23,26)==i]%>%
                raster()%>%
                calc(fun=function(x){ifelse(x>0,ifelse(x>130,1,x/130),0)})%>%
