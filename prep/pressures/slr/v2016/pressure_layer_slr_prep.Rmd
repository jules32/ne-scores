...
months_coast <- function(x){
  ...
  writeRaster(r,filename=paste0(dir_git,'/prep/pressures/slr/v2016/int/msla_monthly_coast/msla_monthly_coast_',m_yr,'.tif'),overwrite=T)
...
month_files <- list.files('int/msla_monthly_coast',full.names=T)
...
foreach (j = all_yrs) %dopar% {
  ...
  rast_annual_mean <- stack(msla_yr)%>%
                      calc(mean,na.rm=T)%>%
                      writeRaster(filename = paste0(file.path(dir_git),'/prep/pressures/slr/v2016/int/msla_annual_mean/rast_msla_annual_',j,'.tif'),overwrite=T)
...
annual_files <- list.files('int/msla_annual_mean',full.names=T)%>%stack()
...
Take each year and rescale from 0 to 1 using reference point. Decreases are set to 0
...
yr <- substr(names(annual_files[[i]]),6,9)
  ...
  annual_files[[i]]%>%
    calc(.,fun=function(x){ifelse(x<0,0,x/ref)},
         filename = paste0('~/github/ohi-northeast/prep/pressures/slr/v2016/int/msla_annual_rescaled/msla_annual_rescaled_',yr,'.tif'),overwrite=T)
...
resc_files <- list.files('~/github/ohi-northeast/prep/pressures/slr/v2016/int/msla_annual_rescaled',full.names=T)%>%stack()
...
pressure_stack <- lapply(list.files('int/msla_annual_rescaled',full.names=T),raster)%>%brick()
...
write.csv(slr_data, file.path("int/slr.csv"), row.names=FALSE)
...
saveData <- function(newYear){
  ...
  write.csv(slr, file.path(sprintf('out/slr_%s.csv', newYear)), row.names=FALSE)
}


### extract data 
for(newYear in (max(slr_data$year) - 10):(max(slr_data$year))){
  saveData(newYear)
}
```
...
now <- slr_data%>%filter(year==2015)

rgns_slr <- rgns
rgns_slr@data <- rgns_slr@data%>%left_join(now)

map <- subs(ocean_rgns,rgns_slr@data, by = "rgn_id", which = "pressure_score")

plot(ocean_ne,col='lightblue',axes=F,legend=F,main = "SLR Pressure Score 2015")
