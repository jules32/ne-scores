...
state_wa <- st_read(dsn = file.path(dir_anx,'spatial'), layer = 'StateWaters_wo_rivs_cleaned')%>%
              st_transform(p4s_nad83)
```
...
ca_eez  <- st_read(dsn = file.path(dir_M,'git-annex/globalprep/spatial/d2014/data'),layer = 'regions_gcs')%>%
...
epu <- st_read(file.path(dir_anx, 'spatial/data_for_rgn_options/Extended_EPU'),'EPU_extended')%>%
        st_transform(p4s_nad83)%>%
...
ggplot() +
  ...
  ggtitle("Option 1")+ 
  labs(fill='Regions') +
 scale_fill_manual(values = c("#A50026","#313695", "#ABD9E9",  "#FEE090", "#D73027", "#E0F3F8","#F46D43",  "#FDAE61", "#74ADD1", "#4575B4"))+ 
...
ggplot() +
  ...
  ggtitle("Option 2")+ 
  labs(fill='Regions') +
 scale_fill_manual(values = c("#313695", "#ABD9E9", "#E0F3F8","#4575B4","#FEE090","#FDAE61"))+ 
  theme(legend.text=element_text(size=12),
        legend.title = element_text(size = 14))
```
...
ggplot() +
  ...
  ggtitle("Option 3")+ 
  labs(fill='Regions') +
 scale_fill_manual(values = c("#FEE090","#313695", "#ABD9E9", "#E0F3F8","#4575B4"))+ 
  theme(legend.text=element_text(size=12),
        legend.title = element_text(size = 14))
```
